...
async function loadCoachData(id) {
    try {
        ...
        // --- Aggiornamento Condizioni (COMPATIBILITÀ TOTALE) ---
        // Funziona sia con 'icon' (singola) che con 'icons' (lista multipla)
        ...
        // Aggiorna passiva allenatore (che ora legge anche il VAL2 se presente)
        updatePassiveText();

    } catch (err) {
        console.error("Errore caricamento allenatore:", err);
    }
}
...
function updatePassiveText() {
    ...
    // Supporta sia la passiva standard ({VAL}) che le passive combinate ({VAL} e {VAL2})
    let parsedText = db.coachPassive.template.replaceAll('{VAL}', activeLevelData.val);
    if (activeLevelData.val2) {
        parsedText = parsedText.replaceAll('{VAL2}', activeLevelData.val2);
    }
